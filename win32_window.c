...
#include <windowsx.h>
#include <shellapi.h>
#include <stdio.h> // added for reading stdin to get parent window handle.
                              ...
                              const _GLFWwndconfig* wndconfig)
{
	...
	/*
		Section inserted by jphg to allow parent handle to be passed in via stdin
	*/
	...
	int parentHWND;
	int wparent = 0;
	if (ReadFile(hStdin, line, sizeof(line), &dwRead, NULL)) {
		//WriteFile(hStdout, line, sizeof(line), &dwWrite, NULL);
		if (1 == sscanf(line, "%x", &parentHWND)) {
			// Configure window styles for embedded window
